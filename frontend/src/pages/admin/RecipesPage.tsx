...
import {
  ...
  Grid, IconButton, Chip, Dialog, DialogTitle, DialogContent, DialogActions,
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: '#fafafa' }}>
        ...
        <Container maxWidth="lg">
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Typography variant="h5" sx={{ fontWeight: 600 }}>Recettes</Typography>
            </Box>
            ...
            <Box sx={{ display: 'flex', gap: 1, mb: 2, flexWrap: 'wrap' }}>
              ...
              {allCats.map(cat => (
                <Chip key={cat} label={cat}
                  ...
                  onClick={() => setCategoryFilter(cat)} clickable
                />
              ))}
            </Box>
          ) : null
            ...
            {filtered.map((recipe) => (
              <Grid item xs={12} sm={6} md={4} key={recipe._id}>
                <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                  {recipe.images?.[0] ? (
                    <CardMedia component="img" height="180" image={recipe.images[0]} alt={recipe.name} />
                  ) : (
                  ...
                  <CardContent sx={{ flexGrow: 1 }}>
                    <Typography variant="h6" sx={{ fontWeight: 600, mb: 1 }}>{recipe.name}</Typography>
                    ...
                    <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap', mb: 2 }}>
                      ...
                      {recipe.variants?.map((v: any, i: number) => (
                        <Chip key={i} label={v.sizeName} size="small" color="primary" variant="outlined" />
                      ))}
                    </Box>
                  </CardContent>
                  <Box sx={{ display: 'flex', justifyContent: 'flex-end', px: 1, pb: 1 }}>
                    <IconButton size="small" title="Modifier" onClick={() => navigate(`/admin/recipes/${recipe._id}/edit`)}>
                      <Edit fontSize="small" />
                    </IconButton>
                    <IconButton size="small" title="Dupliquer" onClick={() => handleDuplicate(recipe._id)}>
                      <ContentCopy fontSize="small" />
                    </IconButton>
                    <IconButton size="small" title="Supprimer" onClick={() => setDeleteId(recipe._id)}>
      ...
      <Dialog open={!!deleteId} onClose={() => setDeleteId(null)}>
        ...
        <DialogActions>
          ...
          <Button onClick={handleDelete} color="error" variant="contained">Desactiver</Button>
        </DialogActions>
      </Dialog>
    </Box>
  )
