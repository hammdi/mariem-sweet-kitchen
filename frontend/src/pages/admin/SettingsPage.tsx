...
const SettingsPage = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try { setPrefs(prev => ({ ...prev, ...JSON.parse(saved) })) } catch { /* ignore */ }
    }
  }, [])
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#fafafa' }}>
        ...
        <Container maxWidth="lg">
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Typography variant="h5" sx={{ fontWeight: 600 }}>Parametres</Typography>
            </Box>
      ...
      <Container maxWidth="md" sx={{ py: 3 }}>
        ...
        <Card sx={{ mb: 3 }}>
          <CardContent>
            <Typography variant="h6" sx={{ mb: 3, fontWeight: 600 }}>Calcul des prix</Typography>
            <Grid container spacing={3}>
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Tarif STEG (DT/kWh)" type="number"
                  helperText="Prix moyen electricite Tunisie"
                  value={pricing.stegTariff}
                  onChange={e => setPricing({ ...pricing, stegTariff: parseFloat(e.target.value) || 0 })}
                  inputProps={{ step: 0.001 }}
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Marge effort (%)" type="number"
                  helperText="Pourcentage ajoute au total"
                  value={pricing.marginPercent}
                  onChange={e => setPricing({ ...pricing, marginPercent: parseFloat(e.target.value) || 0 })}
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Forfait eau - Petit (DT)" type="number"
                  helperText="Pour les recettes <= 8 portions"
                  value={pricing.waterForfaitSmall}
                  onChange={e => setPricing({ ...pricing, waterForfaitSmall: parseFloat(e.target.value) || 0 })}
                  inputProps={{ step: 0.1 }}
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Forfait eau - Grand (DT)" type="number"
                  helperText="Pour les recettes > 8 portions"
                  value={pricing.waterForfaitLarge}
                  onChange={e => setPricing({ ...pricing, waterForfaitLarge: parseFloat(e.target.value) || 0 })}
                  inputProps={{ step: 0.1 }}
                />
              </Grid>
            </Grid>
        ...
        <Card sx={{ mb: 3 }}>
          <CardContent>
            <Typography variant="h6" sx={{ mb: 3, fontWeight: 600 }}>Informations du commerce</Typography>
            <Grid container spacing={3}>
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Nom du commerce" value={prefs.businessName}
                  onChange={e => setPrefs({ ...prefs, businessName: e.target.value })}
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Telephone" value={prefs.businessPhone}
                  onChange={e => setPrefs({ ...prefs, businessPhone: e.target.value })}
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Numero WhatsApp" value={prefs.whatsappNumber}
                  helperText="Affiche sur le site pour les clients"
                  onChange={e => setPrefs({ ...prefs, whatsappNumber: e.target.value })}
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Devise" value={prefs.currency}
                  onChange={e => setPrefs({ ...prefs, currency: e.target.value })}
                />
              </Grid>
              <Grid item xs={12}>
                <TextField fullWidth label="Adresse" value={prefs.businessAddress}
        ...
        <Card sx={{ mb: 3 }}>
          <CardContent>
            <Typography variant="h6" sx={{ mb: 3, fontWeight: 600 }}>Notifications</Typography>
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <FormControlLabel
                  control={<Switch checked={prefs.telegramEnabled}
              ...
              {prefs.telegramEnabled && (
                <Grid item xs={12} sm={6}>
                  <TextField fullWidth label="Telegram Chat ID" value={prefs.telegramChatId}
        ...
        <Card sx={{ mb: 3 }}>
          <CardContent>
            <Typography variant="h6" sx={{ mb: 3, fontWeight: 600 }}>Preferences d'interface</Typography>
            <Grid container spacing={3}>
              <Grid item xs={12} sm={6}>
                <FormControl fullWidth>
                  <InputLabel>Langue</InputLabel>
                  <Select value={prefs.language} label="Langue"
              ...
              <Grid item xs={12} sm={6}>
                <FormControlLabel
                  control={<Switch checked={prefs.darkMode}
