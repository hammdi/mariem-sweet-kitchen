    ...
    <Box sx={{ minHeight: '100vh', bgcolor: '#fafafa' }}>
        ...
        <Container maxWidth="lg">
          <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'stretch', sm: 'center' }, gap: 1 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Typography variant="h5" sx={{ fontWeight: 600 }}>Ingredients</Typography>
            </Box>
      ...
      <Container maxWidth="lg" sx={{ py: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1, mb: 2, flexWrap: 'wrap' }}>
          <Chip label="Tous" color={categoryFilter === '' ? 'primary' : 'default'}
            variant={categoryFilter === '' ? 'filled' : 'outlined'}
            onClick={() => setCategoryFilter('')} clickable
          />
          {categories.map(c => (
            <Chip key={c.value} label={`${c.label} (${ingredients.filter(i => i.category === c.value).length})`}
            ...
            <TableBody>
              {ingredients
                ...
                <TableRow key={ing._id}>
                  ...
                  <TableCell>
                    <Chip
                      label={categories.find(c => c.value === ing.category)?.label || ing.category}
                      size="small" variant="outlined"
                    />
                  </TableCell>
                  <TableCell align="right">
                    <IconButton size="small" onClick={() => openEdit(ing)}><Edit fontSize="small" /></IconButton>
                    <IconButton size="small" onClick={() => setDeleteId(ing._id)}><Delete fontSize="small" /></IconButton>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <TextField fullWidth label="Nom" value={form.name}
            ...
            sx={{ mt: 1, mb: 2 }}
          />
          <TextField fullWidth label="Prix par unite (DT)" type="number" value={form.pricePerUnit}
            onChange={e => setForm({ ...form, pricePerUnit: e.target.value })}
            sx={{ mb: 2 }}
          />
          <FormControl fullWidth sx={{ mb: 2 }}>
            <InputLabel>Unite</InputLabel>
            <Select value={form.unit} label="Unite"
