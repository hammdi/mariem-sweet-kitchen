...
import {
  Container, Typography, Box, Grid, TextField, Card, CardContent, CardMedia,
  Button, Chip, FormControl, InputLabel, Select, MenuItem,
  InputAdornment, Skeleton, Pagination,
} from '@mui/material'
...
const RecipesPage = () => {
  ...
  const load = async (targetPage = page) => {
    ...
    try {
      ...
      const cats = [...new Set(data.flatMap((r: any) => r.categories || []).filter(Boolean))] as string[]
      if (cats.length > 0 && categories.length === 0) setCategories(cats)
    } catch { /* ignore */ }
    setLoading(false)
  }

  useEffect(() => { setPage(1); load(1) }, [category])
          ...
          <Box component="form" onSubmit={handleSearch}>
            <Grid container spacing={2} alignItems="center">
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Rechercher une recette" value={search}
              ...
              <Grid item xs={12} sm={3}>
                <FormControl fullWidth size="small">
                  ...
                  <Select label="Categorie" value={category}
        ...
        {loading ? (
              ...
              {recipes.map((recipe) => {
                ...
                return (
                  <Grid item xs={12} sm={6} md={4} key={recipe._id}>
                    ...
                    >
                      {recipe.images?.[0] ? (
                        <CardMedia component="img" height="200" image={recipe.images[0]} alt={recipe.name}
                          sx={{ objectFit: 'cover' }}
                        />
                      ) : (
                        <Box sx={{ height: 200, bgcolor: 'linear-gradient(135deg, #fef7ee, #fad7a5)',
                      ...
                      <CardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                        <Typography variant="h6" sx={{ fontWeight: 600, mb: 0.5 }}>{recipe.name}</Typography>
                        ...
                        <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap', mb: 2 }}>
                          {recipe.categories?.map((cat: string, ci: number) => (
                            <Chip key={ci} label={cat} size="small" color="primary" variant="outlined" />
                          ))}
          ...
          <Box sx={{ textAlign: 'center', py: 8 }}>
            ...
            <Typography variant="h6" color="text.secondary">
              {search || category ? 'Aucune recette trouvee pour cette recherche' : 'Les recettes arrivent bientot'}
            </Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
              {search || category ? 'Essayez avec d\'autres criteres' : 'Mariem prepare ses meilleures creations'}
            </Typography>
          </Box>
        )}
      </Container>
