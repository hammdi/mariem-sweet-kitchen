...
router.post('/', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (requestedDate) {
    ...
    if (parsedRequestedDate < earliestAllowed) {
      throw createError(
        `Il faut commander au moins ${minLeadHours}h a l'avance`,
        400
      );
    }
...
router.post('/manual', authenticate, authorize('admin'), asyncHandler(async (req: Request, res: Response) => {
  const { clientName, clientPhone, items, notes, requestedDate, additionalFees, saveAsRecipe } = req.body;
  ...
  for (const item of items) {
    ...
    if (item.recipeId) {
      const recipe = await Recipe.findById(item.recipeId);
      if (!recipe) {throw createError(`Recette ${item.recipeId} non trouvee`, 400);}
      if (!recipe.variants[item.variantIndex]) {throw createError(`Taille invalide pour ${recipe.name}`, 400);}

      const price = await PriceCalculationService.calculateVariantPrice(
        item.recipeId, item.variantIndex, item.clientProvidedIngredients || []
      );
      ...
      orderItems.push({
        ...
        calculatedPrice: { ingredientsCost: price.ingredientsCost, electricityCost: price.electricityCost, waterCost: price.waterCost, margin: price.margin, total: price.total },
      });
    }
    // Mode 2 : recette custom (créée à la volée)
      ...
      const price = await PriceCalculationService.calculateVariantPrice(
        newRecipe._id.toString(), 0, item.clientProvidedIngredients || []
      );
  ...
  TelegramService.notifyNewOrder({
    ...
    items: order.items.map((item: any) => {
      const recipe = item.recipeId;
      return { recipeName: recipe?.name || 'Recette', sizeName: 'Custom', quantity: item.quantity, unitPrice: item.calculatedPrice?.total || 0 };
    }),
  });
...
router.get('/shopping-list', authenticate, authorize('admin'), asyncHandler(async (_req: Request, res: Response) => {
  const orders = await Order.find({ status: 'paid' })
    .populate({
      path: 'items.recipeId',
      populate: [{ path: 'variants.ingredients.ingredientId', select: 'name pricePerUnit unit stockQuantity' }]
    });

  // Agréger les besoins par ingrédient
  const needs: Record<string, { ingredientId: string; name: string; unit: string; needed: number; inStock: number; pricePerUnit: number }> = {};
  ...
  const shoppingList = Object.values(needs)
    .map(item => ({
      ...
      estimatedCost: Math.round(Math.max(0, item.needed - item.inStock) * item.pricePerUnit * 1000) / 1000,
    }))
...
router.post('/shopping-list/purchase', authenticate, authorize('admin'), asyncHandler(async (req: Request, res: Response) => {
  ...
  for (const p of purchases) {
    if (!p.ingredientId || !p.quantity || p.quantity <= 0) {continue;}
...
router.get('/check-preparable', authenticate, authorize('admin'), asyncHandler(async (_req: Request, res: Response) => {
...
router.get('/stock-history', authenticate, authorize('admin'), asyncHandler(async (req: Request, res: Response) => {
...
router.put('/:id', authenticate, authorize('admin'), asyncHandler(async (req: Request, res: Response) => {
  ...
  if (req.body.confirmedDate !== undefined) {order.confirmedDate = req.body.confirmedDate ? new Date(req.body.confirmedDate) : null;}
  if (req.body.requestedDate !== undefined) {order.requestedDate = req.body.requestedDate ? new Date(req.body.requestedDate) : null;}
  ...
  if (req.body.ingredientsReady === true && !order.ingredientsReady) {
    ...
    if (!populated) {throw createError('Commande non trouvee', 404);}
    ...
    for (const item of populated.items) {
      ...
      for (const vi of variant.ingredients) {
        ...
        await StockHistory.create({
          ingredientId: ing._id, ingredientName: ing.name, quantity: needed, unit: vi.unit,
          orderId: order._id, clientName: order.clientName, recipeName: recipe.name, type: 'deduction',
        });
      }
    }
    ...
    logger.info(`Ingredients confirmes + stock deduit + preparation lancee pour commande ${order._id}`);
